...
// Массивы, отвечающий за игру
char field[heightY + 2][widthX + 3]; // field[y(строка)][x()]
unsigned short snakeX[200], snakeY[200]; // Координаты змеи

// ФУНКЦИИ:
// Фукции настройки главного меню и т. д.
// void Clock(int tm);
void Settings(bool &walls, bool &SCrossing, short &key); // Меню настройки пользователем
void InputSettings(bool &Switch, short &key); // Ввод настроек
void Setup(unsigned short *snakeX, unsigned short *snakeY,
		   ...
		   const char &SumbolSnakeHead, const char &SumbolSnakeTrunk); // Начальная настрой игры
void ShuttingDown(); // Завершение игры (основной или самой игры?)

// Функции игры
void Draw(const unsigned long &score, const unsigned short &len,
		  const unsigned short &applec); // Рисует поле заданных размеров и выводит дополнительную информацию

void Input(short &_x, short &_y, bool &GameOver); // Управление (ввод) через стрелочки

void GenerationObjects(const unsigned short &applec, unsigned short &OldApplec,
					   unsigned short &appleX, unsigned short &appleY,
					   const char &SumbolField, const char &SumbolApple); // Генерация предметов (яблоки, $, временная защита и мины)
...
void Move(unsigned short *snakeX, unsigned short *snakeY, const short &_x, const short &_y,
		  ...
		  const char &SumbolSnakeHead, const char &SumbolSnakeTrunk); // Направление движение зависит от предыдущей нажатой клавиши
...
void Logic(unsigned short *snakeX, unsigned short *snakeY,
		   ...
		   const char &SumbolWall, const char &SumbolSnakeHead); // Логическая функция отвечает за происходящее после врезания в стену, съедания чего-либо
...
int main()
{
	...
	short key = 80; // Направление змеи (скан-код), пауза (остальные настройки в отдельную функцию)

	// Координатные переменные
	short _x = 0, _y = 1; // Передвижение змеи на определённое количество шагов на следующей итерации цикла
	unsigned short appleX, appleY; // Координаты яблока

	// Переменные-счётчики
	unsigned short len = 2; // Длина змеи
	unsigned long score = 0; // Результат
	unsigned short applec = 0; // Счёт яблок
	unsigned short OldApplec = 1; // OldApplec = applec - 1 (для генерации нового яблока)
	// unsigned long iteration = 0; // Количество итераций за одну игру
	...
	do
	{
		...
		do
		{
			...
			Draw(score, len, applec); // нужен динамический двумерный массив
			...
			Sleep(80); // Можно в настройках увеличить скорость или со следующим уровнем (3 уровня)
		} while ((!GameOver) && (key != 27));

		if (GameOver) // Потом сделать отдельную функцию
		{
			bool Switch = TRUE; // переделать
...
{
	bool Switch = TRUE; // переменная переключатель
...
void Input(short &_x, short &_y, bool &GameOver)
{
	if (_kbhit())
	{
		unsigned short key = 80; // Направление змеи (скан-код) и пауза
